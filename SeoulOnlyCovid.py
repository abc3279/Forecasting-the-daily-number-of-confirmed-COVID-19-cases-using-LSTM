...
scale_cols = ['서울시 추가 확진']

ac_cols = ['서울시 추가 확진']

scaled_df = scaler.fit_transform(raw_df[scale_cols]) # 정규화 시 numpy로 return한다.
s1 = scaler.fit_transform(raw_df[ac_cols])

print(type(scaled_df))

scaled_df = pd.DataFrame(scaled_df, columns=scale_cols) # 이를 padnas DataFrame으로 사용하고 싶다면 DataFrame으로 변경해줘야한다.

print(scaled_df)

feature_cols = ['서울시 추가 확진'] # 학습
label_cols = ['서울시 추가 확진'] # 정답
...
label_np = label_df.to_numpy() # 딥러닝 학습을 위해 DataFrame to Numpy 변환
feature_np = feature_df.to_numpy() # 딥러닝 학습을 위해 DataFrame to Numpy 변환
...
def make_sequence_dataset(feature, label, window_size):
  ...
  return np.array(feature_list), np.array(label_list) # feature list 가 2차원 행렬인데 np.array 를 또 씌워주어서 3차원 형태로
...
file = open("SeoulOnlyCovidHistory.txt", "a")
